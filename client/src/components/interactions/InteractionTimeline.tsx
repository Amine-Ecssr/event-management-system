...
interface InteractionTimelineProps {
  ...
  onEditInteraction?: (interaction: BaseInteraction) => void;
  onDeleteInteraction?: (interaction: BaseInteraction) => void;
}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-medium text-lg">
            {t(`${translationPrefix}.interactions.timeline`)}
          </h3>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {t(`${translationPrefix}.interactions.filterSearch`)}
            {hasActiveFilters && (
              <Badge
                variant="secondary"
        ...
        <div className="p-4 bg-muted/50 rounded-lg space-y-3 border">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="relative flex-1">
              ...
              <Input
                placeholder={t(`${translationPrefix}.interactions.searchPlaceholder`)}
                value={searchQuery}
            ...
            >
              <SelectTrigger className="w-full sm:w-[180px]">
                <SelectValue placeholder={t(`${translationPrefix}.interactions.filterByType`)} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">
                  {t(`${translationPrefix}.interactions.allTypes`)} ({interactions.length})
                </SelectItem>
                {interactionTypes.map((type) => (
                  <SelectItem key={type} value={type}>
                    <div className="flex items-center gap-2">
                      {t(`${translationPrefix}.interactions.types.${type}`)}
                      {typeCounts[type] && (
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="newest">
                  {t(`${translationPrefix}.interactions.newestFirst`)}
                </SelectItem>
                <SelectItem value="oldest">
                  {t(`${translationPrefix}.interactions.oldestFirst`)}
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          {hasActiveFilters && (
            ...
            >
              ...
              {t(`${translationPrefix}.interactions.clearFilters`)}
            </Button>
          )}
        </div>
      )}
      ...
      {interactions.length === 0 ? (
        <div className="text-center py-12 border-2 border-dashed rounded-lg">
          ...
          <h4 className="font-medium text-lg mb-1">
            {t(`${translationPrefix}.interactions.noInteractions`)}
          </h4>
          <p className="text-sm text-muted-foreground mb-4">
            {t(`${translationPrefix}.interactions.noInteractionsDesc`)}
          </p>
          {onAddInteraction && (
            <Button onClick={onAddInteraction}>
              <Plus className="h-4 w-4 mr-1.5" />
              {t(`${translationPrefix}.interactions.addFirstInteraction`)}
            </Button>
          )}
        </div>
        ...
        <div className="text-center py-12 border-2 border-dashed rounded-lg">
          ...
          <h4 className="font-medium text-lg mb-1">
            {t(`${translationPrefix}.interactions.noResults`)}
          </h4>
          <p className="text-sm text-muted-foreground mb-4">
            {t(`${translationPrefix}.interactions.noResultsDesc`)}
          </p>
                  ...
                  <div className="relative z-10 flex flex-col items-center">
                    <div
                      className={cn(
                        ...
                        INTERACTION_ICON_BG[interaction.type] || INTERACTION_ICON_BG.other
                      )}
                    >
