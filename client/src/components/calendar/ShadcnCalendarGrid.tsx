...
interface ShadcnCalendarGridProps {
  currentDate: Date;
  events: Event[];
...
export default function ShadcnCalendarGrid({ 
  currentDate, 
  events, 
  onEventClick, 
  hoveredEventId, 
  onEventHover 
}: ShadcnCalendarGridProps) {
  ...
  const eventColorMap = useMemo(() => {
    ...
    events.forEach((event) => {
      if (!map.has(event.id)) {
        map.set(event.id, getEventColor(event.id));
      }
    });
    return map;
  }, [events]);
  ...
  const eventFirstAppearance = useMemo(() => {
      ...
      events.forEach((event) => {
        ...
        if (eventStart <= weekEnd && eventEnd >= weekStart) {
          if (!appearances.has(event.id)) {
            appearances.set(event.id, weekIndex);
          }
        }
      });
    });
  ...
  const calculateWeekHeight = (rowCount: number, isMobile: boolean): string => {
    const baseHeight = isMobile ? 112 : 80;
    const rowHeight = 26; // Slightly taller for shadcn styling
    const additionalHeight = Math.max(0, rowCount) * rowHeight;
    return `${baseHeight + additionalHeight}px`;
  };

  return (
    <Card className="overflow-hidden" data-testid="shadcn-calendar-grid">
      {/* Weekday headers */}
            ...
            <div key={weekIndex} className="relative">
                ...
                {week.map((day, dayIndex) => {
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        "border-r border-b p-1 md:p-2 transition-colors",
                        isCurrentMonth ? "bg-background" : "bg-muted/30",
                        isCurrentDay && "bg-primary/5",
                        dayIndex === 6 && "border-r-0"
                      )}
                      data-testid={`shadcn-calendar-day-${format(day, 'yyyy-MM-dd')}`}
                    >
                      <div
                        className={cn(
                          "flex items-center justify-center w-7 h-7 text-xs md:text-sm font-medium rounded-full transition-colors mb-1",
              ...
              <div className="hidden md:block absolute inset-0 pointer-events-none">
                ...
                <div className="grid grid-cols-7 h-full p-1 md:p-2">
                  <div className="col-span-7 pt-7 md:pt-8">
                    <div className="grid grid-cols-7 gap-y-1">
                      {eventBars.map((bar, barIndex) => {
                        ...
                        const eventName = isArabic && bar.event.nameAr ? bar.event.nameAr : bar.event.name;
                        const eventLocation = isArabic && bar.event.locationAr ? bar.event.locationAr : bar.event.location;
                        
                        return (
                          <HoverCard key={`${bar.event.id}-${weekIndex}`} openDelay={200} closeDelay={100}>
                            <HoverCardTrigger asChild>
                              <div
                                ...
                                className={cn(
                                  "text-xs px-2 h-6 flex items-center cursor-pointer pointer-events-auto transition-all mx-0.5",
                                  bar.span === 1 ? "rounded-md" : isFirstDayOfEvent ? "rounded-l-md" : "rounded-md",
                                  hoveredEventId === bar.event.id && "ring-2 ring-ring ring-offset-1 ring-offset-background shadow-md"
                                )}
                                data-testid={`shadcn-event-bar-${bar.event.id}`}
                              >
                            ...
                            <HoverCardContent className="w-80" side="top" align="start">
                              <div className="space-y-2">
                                <h4 className="text-sm font-semibold">{eventName}</h4>
                                ...
                                {bar.event.category && (
                                  <Badge variant="secondary" className="text-xs">
                                    {isArabic && bar.event.categoryAr ? bar.event.categoryAr : bar.event.category}
                                  </Badge>
                                )}
                              </div>
                            </HoverCardContent>
                          ...
                          <Tooltip key={`${bar.event.id}-${weekIndex}-mobile`}>
                            <TooltipTrigger asChild>
                              <div
                                ...
                                className={cn(
                                  "text-xs px-1.5 h-5 flex items-center cursor-pointer pointer-events-auto transition-all",
                                  bar.span === 1 ? "rounded-md" : isFirstDayOfEvent ? "rounded-l-md" : "rounded-md",
                                  hoveredEventId === bar.event.id && "ring-1 ring-ring"
                                )}
                                data-testid={`shadcn-event-bar-mobile-${bar.event.id}`}
                              >
                                <span className="truncate font-medium text-foreground">
                                  {eventName}
                                </span>
